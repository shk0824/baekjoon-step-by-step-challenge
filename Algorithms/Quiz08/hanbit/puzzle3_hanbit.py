...
def InviteDinnerOptimized(guestList, dislikePairs):
    ...
    for i in range(2**n):
        ...
        pairs=0 #이번엔 good 이 아니라, 들어가면 안되는 친구들 쌍이 몇개 있는지 헤아리기
        for j in dislikePairs:
            if j[0] in Combination and j[1] in Combination:
                pairs+=1 #쌍이 발견될 때마다 올리기
        if pairs<=1: #만약 쌍이 1과 같거나 작으면
    ...
    for i in guestList:
        for j in invite:
            if i[0]==j:
                OptimumList+=[i] # invite에서 이름과 점수를 찾아서 넣기
    for i in goodfriend: #마지막으로 들어가야 되는 친구들 추가
        OptimumList+=[i]
        TotalScore+=i[1]
    OptimumList.sort(key=lambda x: x[0]) # 알파벳 순으로 정렬

    print('Optimum Solution: ', OptimumList, "\nWeight is: ",TotalScore)
